...
def spellcheck(df):
    changed = [check(x) for x in df['search_term']]
    df['spell_corrected'] = [1 if x != y else 0 for x,y in zip(changed, df['search_term'])]
    df['search_term'] = [x for x in changed]
...
if ('train' in PREPROCESS_THIS):
    ...
    print("spell correcting training search term")
    spellcheck(df_train)
...
if ('test' in PREPROCESS_THIS):
    ...
    print("spell correcting test search terms")
    spellcheck(df_test)
